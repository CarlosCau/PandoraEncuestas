...
$(document).ready(function(){
    ...
    icnEncuestas = L.icon({iconUrl:'images/encuesta.svg',iconSize:[35,35]});
    
    refreshEncuestas();
    
   function returnEncuestasMarker(json, latlng){
        var att = json.properties;
        arEncuestas.push(att.id.toString());
        return L.marker(latlng, {icon:icnEncuestas}).bindPopup("<tr><td><b>Encuestador: </b></td><td>"+att.usuario+"</td>");
    }           

    function refreshEncuestas(){
        $.ajax({url:'php/load_data.php',
                data:{tbl:'respuestas_cuest',flds:"id, usuario"},
                type: 'POST',
               success: function(response){
                   ...
                   arEncuestas=[];
                   jsnEncuestas = JSON.parse(response);
                   if (lyrEncuestasCluster){
                       ctlLayers.removeLayer(lyrEncuestasCluster);
                       lyrEncuestasCluster.remove();
                       lyrEncuestas.remove();
                   }
                   lyrEncuestas = L.geoJSON(jsnEncuestas,{pointToLayer:returnEncuestasMarker});
                    arEncuestas.sort(function(a,b){return a-b});

                    lyrEncuestasCluster = L.markerClusterGroup({
                        spiderfyOnMaxZoom: true,
                        showCoverageOnHover: true,
                        zoomToBoundsOnClick: true,
                        disableClusteringAtZoom: 22
                    });
                    lyrEncuestasCluster.clearLayers();
                    lyrEncuestasCluster.addLayer(lyrEncuestas);
                    lyrEncuestasCluster.addTo(mymap);
                    ctlLayers.addOverlay(lyrEncuestasCluster,"Encuestas Levantadas");
                   }
               },
